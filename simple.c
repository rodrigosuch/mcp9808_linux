...
MODULE_AUTHOR("Rodrigo Such");
MODULE_LICENSE("Dual BSD/GPL");

#define I2C_BUS_AVAILABLE   ( 2 )           // I2C Bus available in our Raspberry Pi
#define SLAVE_DEVICE_NAME   ( "MCP9808" )   // Device and Driver Name
#define MCP9808_HW_ADDR  	( 0 )			// MCP9808 hardware (pin) controled address component
#define MCP9808_SLAVE_ADDR  ( 0x18 | MCP9808_HW_ADDR) // MCP9808 address

static struct i2c_adapter *etx_i2c_adapter     = NULL;  // I2C Adapter Structure
static struct i2c_client  *etx_i2c_client_temp = NULL;  // I2C Cient Structure (In our case it is OLED)
/*
** I2C Board Info strucutre
*/
static struct i2c_board_info oled_i2c_board_info = {
        I2C_BOARD_INFO(SLAVE_DEVICE_NAME, MCP9808_SLAVE_ADDR)
    };
...
static void __exit mcp9808_driver_exit(void)
{
    i2c_unregister_device(etx_i2c_client_temp);
    pr_info("Driver Removed!!!\n");
}

/** Module Init function */
static int __init mcp9808_driver_init(void)
{
    ...
    etx_i2c_client_temp = i2c_new_client_device(etx_i2c_adapter, &oled_i2c_board_info);
	i2c_put_adapter(etx_i2c_adapter);
	if (etx_i2c_client_temp == NULL) {
		printk("Failed to get new client \n");
		return -EFAULT;
	}
	printk("Init sucessfully \n");
    return 0;
}

module_init(mcp9808_driver_init);
module_exit(mcp9808_driver_exit);
